...
from ._backend_mode import resolve_backend
    ...
    if sh_degree is None:
        ...
        if packed:
            if colors.dim() == num_batch_dims + 2:
                # Turn [..., N, D] into [nnz, D]
                colors = colors.view(B, N, colors.shape[-1])[batch_ids, gaussian_ids]
            else:
...
def _rasterization(
    ...
    .. note::
        This function still relies on ptxsplat's CUDA backend for some computation, but the
...
#     **kwargs,
# ) -> Tuple[Tensor, Tensor, Dict]:
#     """Wrapper for old version ptxsplat.
...
#     """
#     from ptxsplat.cuda_legacy._wrapper import (
#         project_gaussians,
#         rasterize_gaussians,
#         spherical_harmonics,
#     )
