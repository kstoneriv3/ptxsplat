...
VERBOSE=1 FAST_COMPILE=1 TORCH_CUDA_ARCH_LIST="8.9" python -c "from ptxsplat.cuda._backend import _C"
"""
...
# https://github.com/nerfstudio-project/ptxsplat/pull/583#issuecomment-2732597080
if version.parse(torch.__version__) < version.parse("2.2") and USE_PRECOMPILED_HEADERS:
    Console().print(
        "[yellow]ptxsplat: Precompiled headers are enabled but torch version is lower than 2.2. Disabling it.[/yellow]"
    )
...
try:
    ...
    from ptxsplat import csrc as _C
except ImportError:
    ...
    if cuda_toolkit_available():
        ...
        if os.path.exists(os.path.join(build_dir, f"{name}.so")) or os.path.exists(
            ...
            tic = time.time()
            with Console().status(
                f"[bold yellow]ptxsplat: Setting up CUDA with MAX_JOBS={os.environ['MAX_JOBS']} (This may take a few minutes the first time)",
            ...
            toc = time.time()
            Console().print(
                f"[green]ptxsplat: CUDA extension has been set up successfully in {toc - tic:.2f} seconds.[/green]"
            )

    else:
        Console().print(
            "[yellow]ptxsplat: No CUDA toolkit found. ptxsplat will be disabled.[/yellow]"
        )
