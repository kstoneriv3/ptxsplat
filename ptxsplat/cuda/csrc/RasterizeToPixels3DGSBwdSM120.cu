...
struct alignas(16) Sm120GaussianStage {
    float4 xy_opacity_conic_x;
    float4 conic_yz_rgb_rg;
    float4 rgb_b_id_padding;
};
static_assert(sizeof(Sm120GaussianStage) == 3 * sizeof(float4));

template <uint32_t CDIM, typename scalar_t>
__global__ void rasterize_to_pixels_3dgs_bwd_sm120_float4_kernel(
    const uint32_t I,
    const uint32_t N,
    ...
    static_assert(CDIM == 3, "SM120 float4 staging supports RGB only");
    ...
    Sm120GaussianStage *gaussian_batch =
        reinterpret_cast<Sm120GaussianStage *>(s);
...
#pragma unroll
    ...
    float background_dot = 0.f;
    if (backgrounds != nullptr) {
#pragma unroll
        for (uint32_t k = 0; k < CDIM; ++k) {
            background_dot += backgrounds[k] * v_render_c[k];
        }
    }
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        if (idx >= range_start) {
            ...
            gaussian_batch[tr] = {
                make_float4(xy.x, xy.y, opac, conic.x),
                make_float4(conic.y, conic.z, colors[g * 3], colors[g * 3 + 1]),
                make_float4(colors[g * 3 + 2], __int_as_float(g), 0.f, 0.f)
            };
        }
        ...
        for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size;
            ...
            if (valid) {
                const Sm120GaussianStage gaussian = gaussian_batch[t];
                conic = {
                    gaussian.xy_opacity_conic_x.w,
                    gaussian.conic_yz_rgb_rg.x,
                    gaussian.conic_yz_rgb_rg.y
                };
                opac = gaussian.xy_opacity_conic_x.z;
                delta = {
                    gaussian.xy_opacity_conic_x.x - px,
                    gaussian.xy_opacity_conic_x.y - py
                };
            ...
            if (valid) {
                const Sm120GaussianStage gaussian = gaussian_batch[t];
                const float rgb[3] = {
                    gaussian.conic_yz_rgb_rg.z,
                    gaussian.conic_yz_rgb_rg.w,
                    gaussian.rgb_b_id_padding.x
                };
...
#pragma unroll
                ...
                if (backgrounds != nullptr) {
                    v_alpha += -T_final * ra * background_dot;
                }
...
void launch_rasterize_to_pixels_3dgs_bwd_sm120_float4_template(
    // Gaussian parameters
    ...
    int64_t shmem_size =
        tile_size * tile_size * sizeof(Sm120GaussianStage);
    ...
    if (cudaFuncSetAttribute(
            rasterize_to_pixels_3dgs_bwd_sm120_float4_kernel<CDIM, float>,
...
void launch_rasterize_to_pixels_3dgs_bwd_sm120_kernel(
    ...
    launch_rasterize_to_pixels_3dgs_bwd_sm120_float4_template<3>(
        means2d,
        conics,
        colors,
        opacities,
