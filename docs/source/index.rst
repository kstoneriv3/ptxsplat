...
*ptxsplat* is an open-source library for CUDA-accelerated differentiable rasterization of
3D gaussians with Python bindings. It is inspired by the SIGGRAPH paper "3D Gaussian Splatting for
Real-Time Rendering of Radiance Fields" :cite:p:`kerbl3Dgaussians`, but we've made *ptxsplat* even
faster, more memory efficient, and with a growing list of new features!

* *ptxsplat* is developed with efficiency in mind. Comparing to the `official implementation <https://github.com/graphdeco-inria/gaussian-splatting>`_,
  *ptxsplat* enables up to **4x less training memory footprint**, and up to **15% less training time** on Mip-NeRF 360 captures, and potential more on larger scenes. See :doc:`tests/eval` for details.

* *ptxsplat* is designed to **support extremely large scene rendering**, which is magnitudes
...
* *ptxsplat* offers many extra features, including **batch rasterization**,
...
* *ptxsplat* is equipped with the **latest and greatest** 3D Gaussian Splatting techniques,
...
*ptxsplat* is available on PyPI and can be installed with pip:
...
.. code-block:: bash
    ...
    pip install git+https://github.com/nerfstudio-project/ptxsplat
