...
`ptxsplat` is designed with efficiency in mind so it's very suitable to render a large scene.
For example here we mimic a large scene by replicating the Garden scene into a 9x9 grid,
which results 30M Gaussians in total while `ptxsplat` still allows real-time rendering for it.
...
.. code-block:: bash
    ...
    # View it in a viewer with ptxsplat
    python simple_viewer.py --scene_grid 5 --ckpt results/garden/ckpts/ckpt_6999.pt --backend ptxsplat
